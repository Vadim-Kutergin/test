...
def name_table(host,comm):
#    print (host)
    try:
        name=get(host, [sysName], hlapi.CommunityData(comm))[sysName]
        neighbors= walk(host, [cdpCacheDeviceId,cdpCacheDevicePort,cdpCacheCapabilities],hlapi.CommunityData(comm))
        interfaces = get_bulk_auto(host, [ifIndex,ifxName, ifxAlias], hlapi.CommunityData(comm), ifNumber)
...
#    print(neighbors)
    ...
    for i in neighbors:
        ...
        if int.from_bytes(str.encode(l[2]),"big")&0x9 :#если это роутер или свитч
            neig_list[k[0].split('.')[14]]=l#    добавляем с ключом =ifIndex



    for k,v in neig_list.items():
        neig_list[k][0]=replace_all(v[0],short_names)
        neig_list[k][1]=replace_all(v[1],ifc_short_names)


    tbl=[]
    for i in ifc_list:
        ifid=str(i[0])
        if ifid in neig_list:
            rec=neig_list.get(ifid)
